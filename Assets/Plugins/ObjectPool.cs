...
namespace Assets.Plugins
{
    ...
    public class ObjectPool : MonoBehaviour
    {
        ...
        /// </param>
        public GameObject GetObjectForType(string objectType, bool onlyPooled, Vector2 position)
        {
            for (int i = 0; i < Entries.Length; i++)
            {
                ...
                if (!onlyPooled)
                {
                    //return Instantiate(Entries[i].Prefab) as GameObject; // default

                    GameObject newObj = Instantiate(Entries[i].Prefab) as GameObject; // nwz
